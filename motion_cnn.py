...
parser.add_argument('--num_imgs', default=6, type=int, metavar='N', help='number of frames used for optical flow. total number of channels should be twice of this number')
parser.add_argument('--checkpoint_path', default='./checkpoint_motion_flipped', type=str, metavar='PATH', help='path for saving checkpoint')
...
class Motion_CNN():
    def __init__(self, nb_epochs, lr, batch_size, resume, start_epoch, evaluate, train_loader, test_loader, channel,test_video, weights):
    ...
    def build_model(self):
        ...
        #build model
        self.model = resnet101(pretrained=arg.pretrained, channel=self.channel, num_classes = arg.num_classes).cuda()
        #print self.model
        #Loss function and optimizer
        if arg.weight_per_class:
            self.criterion = nn.CrossEntropyLoss(self.weights).cuda()
        else:
